...
export const CalculatorPage = ({ config, items, setItems, user, onLogin, userProfile, theme, aluminumPrices, db, appId }) => {
    ...
    // เพิ่ม State สำหรับจัดการระบบล็อครางม่าน
    const [isRailUnlocked, setIsRailUnlocked] = useState(false);
    const [railPasscodeInput, setRailPasscodeInput] = useState('');
    const [railPasscodeError, setRailPasscodeError] = useState('');
    ...
    // ตรวจสอบเงื่อนไขว่าเมนูรางม่านต้องถูกล็อคหรือไม่
    const isRailLocked = calcMode === 'rail' && !isRailUnlocked && config.railPassword && config.railPassword.trim() !== '';
    ...
    const handleEdit = (item) => {
        if (item.rawParams) {
            ...
            if (p.calcMode === 'rail') setIsRailUnlocked(true);
        } else {
            ...
            if (item.type === 'rail') setIsRailUnlocked(true);
        }
        ...
        <div className="pt-24 px-5 pb-40 fade-in">
            ...
            {availableModes.length > 0 && (
                <div className={`bg-white rounded-[2rem] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] border border-white p-6 mb-6 relative overflow-hidden scale-in ${editingId ? 'ring-2 ring-orange-400' : ''}`}>
                    ...
                    {isRailLocked ? (
                        <div className="bg-slate-50 p-8 rounded-2xl border border-slate-200 text-center animate-scale-in my-2">
                            <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm border border-slate-100"><Icons.Lock size={28} className="text-slate-400" /></div>
                            <h3 className="font-bold text-lg text-slate-700 mb-1">เมนูถูกล็อค</h3>
                            <p className="text-sm text-slate-500 mb-6">กรุณาใส่รหัสผ่านเพื่อเข้าใช้งานเมนูคำนวณรางม่าน</p>
                            <input type="password" value={railPasscodeInput} onChange={e => {setRailPasscodeInput(e.target.value); setRailPasscodeError('');}} placeholder="รหัสผ่าน..." className={`input-apple w-full max-w-[220px] mx-auto bg-white border border-slate-200 rounded-xl p-4 font-bold text-center text-slate-700 mb-2 ${theme.inputRing}`} />
                            {railPasscodeError && <p className="text-xs font-bold text-red-500 mb-4 animate-pulse">{railPasscodeError}</p>}
                            <button onClick={() => {
                                if(railPasscodeInput === config.railPassword) {
                                    setIsRailUnlocked(true);
                                    setRailPasscodeError('');
                                } else {
                                    setRailPasscodeError('รหัสผ่านไม่ถูกต้อง');
                                }
                            }} className={`w-full max-w-[220px] mx-auto py-3.5 rounded-xl font-bold text-sm shadow-md transition-all ${theme.btnPrimary}`}>ปลดล็อค</button>
                        </div>
                    ) : (
                        <>
                            {calcMode === 'rail' && (
